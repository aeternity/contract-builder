...
export default (runWebpackCli: (args: string[]) => Promise<void>): Command => {
  ...
  return new Command()
    ...
    .option('--compiler-path <compilerPath>', 'path to CLI compiler using to build contracts', compilerCliPathPlaceholder)
    .option('--compiler-url <compilerUrl>', 'HTTP compiler URL using to build contracts', defaultCompilerUrl)
    .action(async (
      contracts: string[],
      {
        watch, compilerType, compilerPath, compilerUrl,
      }: {
        watch?: boolean;
